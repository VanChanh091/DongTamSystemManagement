...
class _DeliveryEstimateTimeState extends State<DeliveryEstimateTime> {
                                      ...
                                      ? Row(
                                        children: [
                                          AnimatedButton(
                                            onPressed:
                                                hasSelection
                                                    ? () => _handleClosing(selectedPlanningIds)
                                                    : null,
                                            label: "Hoàn Thành",
                                            icon: Symbols.check,
                                            backgroundColor: themeController.buttonColor,
  ...
  Future<void> _handleRegister(List<int> planningIds) async {
      ...
      onConfirm: () async {
        try {
          final success = await DeliveryService().handlePutDelivery(
            ...
            qtyRegistered: int.parse(registeredController.text),
            note: noteController.text,
          );

          if (success && mounted) {
            showSnackBarSuccess(context, "Xác nhận lên kế hoạch giao hàng thành công");
          ...
          if (mounted) {
            showSnackBarError(context, "Có lỗi khi xác nhận lên kế hoạch giao hàng");
          }
          return false;
        }
      },
  ...
  Future<void> _handleClosing(List<int> planningIds) async {
    final bool confirm = await showConfirmDialog(
      context: context,
      title: "Xác Nhận Đóng Kế Hoạch Này",
      content: "Bạn có chắc chắn muốn đóng kế hoạch này?",
    ...
    if (confirm) {
      try {
        ...
        final bool isBoxType = selectedPapers.any((p) => p.hasBox == true);

        final success = await DeliveryService().handlePutDelivery(
          action: "CLOSE_PLANNING",
          planningId: planningIds,
          isPaper: !isBoxType,
        );

        if (success && mounted) {
          showSnackBarSuccess(context, "Đóng kế hoạch thành công");
          loadPlanningEstimate();
        }
      } on ApiException catch (e) {
        if (mounted) {
          final showError = showSnackBarError(context, e.message!);
          ...
          switch (e.errorCode) {
            ...
            case "NO_INBOUND_HISTORY":
              ...
              break;
            default:
              showSnackBarError(context, e.message ?? "Có lỗi khi đóng kế hoạch");
          }
        }
      } catch (e) {
        if (mounted) {
          showSnackBarError(context, "Có lỗi khi đóng kế hoạch");
        }
      }
    }
  }
