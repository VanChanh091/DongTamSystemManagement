...
class AdminWaveCrestState extends State<AdminWaveCrest> {
                  ...
                  AnimatedButton(
                    onPressed: () async {
                      if (isSelected.isEmpty) {
                        showSnackBarError(context, "Chưa chọn thông tin cần cập nhật");
                        return;
                      }

                      final dataToUpdate =
                          updatedWaveCrest
                              .where((item) => isSelected.contains(item.waveCrestCoefficientId))
                              .toList();
  ...
  Widget _buildTableSection() {
    return FutureBuilder<List<AdminWaveCrestModel>>(
                  ...
                  child: Checkbox(
                    ...
                    onChanged: (value) {
                      setState(() {
                        selectedAll = value!;
                        if (selectedAll) {
                          isSelected = data.map((e) => e.waveCrestCoefficientId).toList();
                        } else {
                          isSelected.clear();
                        }
                      });
            ...
            rows: List<DataRow>.generate(data.length, (index) {
              ...
              return DataRow(
                cells: [
                  ...
                  DataCell(styleCellAdmin(text: waveCrest.machineName.toString(), onChanged: null)),
                ],
              );
            }),
          ),
